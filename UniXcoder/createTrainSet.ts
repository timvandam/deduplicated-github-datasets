    ...
    [
      {
        name: 'datasetFolder',
        optional: false,
        description: 'Folder for a dataset. Must contain ./sets/train.csv',
      },
    ],
    __filename,
  );
...
async function main() {
  ...
  for await (const line of drop(1, lineStream)) {
    ...
    const fileLineStream = createReadLineStream(
      resolve(options.datasetFolder, './repository-files', file),
    );
