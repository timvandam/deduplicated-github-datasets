...
namespace NearCloneDetector
{
    class CloneDetector
    {
        ...
        private IEnumerable<(string Project1, string Project2, int ComparisonIndex, int TotalComparisons)> GetAllProjectCombinations()
        {
            var allProjects = _index.Keys.ToArray();
            int k = 1;
            int totalComparisons = allProjects.Length * (allProjects.Length + 1) / 2;
            for (int i = 0; i < allProjects.Length; i++)
            {
                for (int j = i + 1; j < allProjects.Length; j++)
                {
                    yield return (allProjects[i], allProjects[j], k++, totalComparisons);
                }
                yield return (allProjects[i], allProjects[i], k++, totalComparisons);
            }
        }
        ...
        {
            return GetAllProjectCombinations().AsParallel().SelectMany(projs => FindNearDuplicates(keyJaccardThreshold, jaccardThreshold, projs.Project1, projs.Project2, projs.ComparisonIndex, projs.TotalComparisons));
        }
