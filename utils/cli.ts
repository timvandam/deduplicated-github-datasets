...
export function cli(
  ...
  const positionalArguments = options
    .map(({ name, optional }) => (optional ? `[${name}]` : name))
    .join(' ');
  const usage = `${fileName.endsWith('.ts') ? 'ts-' : ''}node ${relative(
    process.cwd(),
    fileName,
  )} ${positionalArguments}`;
    ...
    options
      .map(
        ({ name, description, defaultValue }) =>
          `\t${name.padEnd(longestCommandLength, ' ')}\t\t${description}` +
